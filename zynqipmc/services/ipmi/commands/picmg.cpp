...
static void ipmicmd_Get_PICMG_Properties(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	ipmb.send(message.prepareReply({
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Address_Info(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Shelf_Address_Info(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Set_Shelf_Address_Info(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_FRU_Control(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
IPMICMD_INDEX_REGISTER(FRU_Control);
#endif

static void ipmicmd_Get_FRU_LED_Properties(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
...
static void ipmicmd_Get_LED_Color_Capabilities(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
...
static void ipmicmd_Set_FRU_LED_State(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	...
	ipmb.send(message.prepareReply({ IPMI::Completion::Success, 0 }));
}
IPMICMD_INDEX_REGISTER(Set_FRU_LED_State);

static void ipmicmd_Get_FRU_LED_State(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	if (message.data_len != 3 || message.data[1] != 0)
		// Not enough parameters, or asking for FRU != 0.  We dont have one of those.
		RETURN_ERROR(ipmb, message, IPMI::Completion::Parameter_Out_Of_Range);
...
#if 0 // Unimplemented.
static void ipmicmd_Set_IPMB_State(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
IPMICMD_INDEX_REGISTER(Set_IPMB_State);
#endif

static void ipmicmd_Set_FRU_Activation_Policy(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	...
	bool act_locked = mstatemachine->activationLocked();
	bool deact_locked = mstatemachine->deactivationLocked();
	if (message.data[2] & 0x01)
		act_locked = message.data[3] & 0x01;
	if (message.data[2] & 0x02)
		deact_locked = message.data[3] & 0x02;
	mstatemachine->setActivationLock(act_locked, deact_locked);
	ipmb.send(message.prepareReply({IPMI::Completion::Success, 0}));
}
IPMICMD_INDEX_REGISTER(Set_FRU_Activation_Policy);

static void ipmicmd_Get_FRU_Activation_Policy(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	if (message.data_len != 2 || message.data[1] != 0)
		// Not enough parameters, or asking for FRU != 0.  We dont have one of those.
		RETURN_ERROR(ipmb, message, IPMI::Completion::Parameter_Out_Of_Range);
	if (!mstatemachine)
		// Not yet initialized (we got an IPMI message before ipmc_service_init() completed)
		RETURN_ERROR(ipmb, message, IPMI::Completion::Node_Busy);
	std::shared_ptr<IPMIMessage> reply = message.prepareReply({IPMI::Completion::Success, 0, 0});
	if (mstatemachine->deactivationLocked())
		reply->data[2] |= 0x02;
	if (mstatemachine->activationLocked())
		reply->data[2] |= 0x01;
	ipmb.send(reply);
}
IPMICMD_INDEX_REGISTER(Get_FRU_Activation_Policy);

static void ipmicmd_Set_FRU_Activation(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	if (message.data_len != 3 || message.data[1] != 0)
		// Not enough parameters, or asking for FRU != 0.  We dont have one of those.
		RETURN_ERROR(ipmb, message, IPMI::Completion::Parameter_Out_Of_Range);
	...
	if (message.data[2] == 0) {
		// Deactivate FRU
		mstatemachine->deactivateFru();
	}
	else if (message.data[2] == 1) {
		// Activate FRU
		mstatemachine->activateFru();
	}
	else {
		// What?
		RETURN_ERROR(ipmb, message, IPMI::Completion::Parameter_Out_Of_Range);
...
static void ipmicmd_Get_Device_Locator_Record_ID(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	...
	for (auto it = device_sdr_repo_vector.begin(), eit = device_sdr_repo_vector.end(); it != eit; ++it) {
		...
		ipmb.send(message.prepareReply({IPMI::Completion::Success, 0, static_cast<uint8_t>(key & 0xff), static_cast<uint8_t>(key >> 8)}));
		return;
	}
	ipmb.send(message.prepareReply({IPMI::Completion::Requested_Sensor_Data_Or_Record_Not_Present}));
}
IPMICMD_INDEX_REGISTER(Get_Device_Locator_Record_ID);

static void ipmicmd_Set_Port_State(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
...
static void ipmicmd_Get_Port_State(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
IPMICMD_INDEX_REGISTER(Get_Port_State);

static void ipmicmd_Compute_Power_Properties(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	if (message.data_len != 2 || message.data[1] != 0)
		// Not enough parameters, or asking for FRU != 0.  We dont have one of those.
		RETURN_ERROR(ipmb, message, IPMI::Completion::Parameter_Out_Of_Range);
	...
	ipmb.send(message.prepareReply({IPMI::Completion::Success, 0, properties.spanned_slots, properties.controller_location}));
}
IPMICMD_INDEX_REGISTER(Compute_Power_Properties);

static void ipmicmd_Set_Power_Level(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	if (message.data_len != 4 || message.data[1] != 0)
...
static void ipmicmd_Get_Power_Level(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
	if (message.data_len != 3 || message.data[1] != 0)
		// Not enough parameters, or asking for FRU != 0.  We dont have one of those.
		RETURN_ERROR(ipmb, message, IPMI::Completion::Parameter_Out_Of_Range);
	...
	std::shared_ptr<IPMIMessage> reply = message.prepareReply({IPMI::Completion::Success, 0, 0, 0, 0});
...
static void ipmicmd_Renegotiate_Power(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Fan_Speed_Properties(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Set_Fan_Level(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Fan_Level(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Bused_Resource(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_IPMB_Link_Info(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Shelf_Manager_IPMB_Address(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Set_Fan_Policy(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Fan_Policy(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_FRU_Control_Capabilities(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_FRU_Inventory_Device_Lock_Control(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_FRU_Inventory_Device_Write(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Shelf_Manager_IP_Addresses(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Shelf_Power_Allocation(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Telco_Alarm_Capability(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Set_Telco_Alarm_State(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Telco_Alarm_State(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Target_Upgrade_Capabilities(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Component_Properties(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Abort_Firmware_Upgrade(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Initiate_Upgrade_Action(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Upload_Firmware_Block(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Finish_Firmware_Upload(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Upgrade_Status(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Activate_Firmware(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Query_Self_Test_Results(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Query_Rollback_Status(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Initiate_Manual_Rollback(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Telco_Alarm_Location(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Set_FRU_Extracted(IPMBSvc &ipmb, const IPMIMessage &message) {
	ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_HPM_x_Capabilities(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Dynamic_Credentials(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_Session_Handle_for_Explicit_LAN_Bridging(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_ATCA_Extended_Management_Resources(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_AMC_Extended_Management_Resources(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Set_ATCA_Extended_Management_State(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_ATCA_Extended_Management_State(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Set_AMC_Power_State(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_AMC_Power_State(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Assign_SOL_Payload_Instance(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
...
#if 0 // Unimplemented.
static void ipmicmd_Get_IP_Address_Source(IPMBSvc &ipmb, const IPMIMessage &message) {
    ASSERT_PICMG_IDENTIFIER(ipmb, message);
}
