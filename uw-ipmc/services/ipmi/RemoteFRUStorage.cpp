 ...
 */
std::string RemoteFRUStorage::RenderTypeLengthField(std::vector<uint8_t> &data) {
	if (data.empty())
...
#define PARSE_TLFIELD(field) \
	do { \
		...
		field = RemoteFRUStorage::RenderTypeLengthField(subvec); \
		parse_offset += 1+len; \
	} while (0);
	...
	while (data[parse_offset] != 0xC1 /* End Marker */) {
		...
		CHECK_OFFSET(len);
		std::vector<uint8_t> subvec(data.begin()+parse_offset, data.begin()+parse_offset+1+len);
		chassis->custom_info.push_back(RemoteFRUStorage::RenderTypeLengthField(subvec));
		parse_offset += 1+len;
	}
#undef PARSE_TLFIELD
#undef CHECK_OFFSET
...
#define PARSE_TLFIELD(field) \
	...
	while (data[parse_offset] != 0xC1 /* End Marker */) {
		...
		CHECK_OFFSET(len);
		std::vector<uint8_t> subvec(data.begin()+parse_offset, data.begin()+parse_offset+1+len);
		board->custom_info.push_back(RemoteFRUStorage::RenderTypeLengthField(subvec));
		parse_offset += 1+len;
	}
#undef PARSE_TLFIELD
#undef CHECK_OFFSET
...
#define PARSE_TLFIELD(field) \
	...
	while (data[parse_offset] != 0xC1 /* End Marker */) {
		...
		CHECK_OFFSET(len);
		std::vector<uint8_t> subvec(data.begin()+parse_offset, data.begin()+parse_offset+1+len);
		product->custom_info.push_back(RemoteFRUStorage::RenderTypeLengthField(subvec));
		parse_offset += 1+len;
	}
#undef PARSE_TLFIELD
#undef CHECK_OFFSET
