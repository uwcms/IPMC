 ...
 */
Telnet::TelnetConsoleSvc::TelnetConsoleSvc(Socket &socket, std::shared_ptr<InputProtocolParser> proto, CommandParser &parser, const std::string &name, LogTree &logtree, bool echo, TickType_t read_data_timeout, std::function<void(TelnetConsoleSvc&)> shutdown_complete_cb)
...
ssize_t Telnet::TelnetConsoleSvc::raw_read(char *buf, size_t len, TickType_t timeout, TickType_t read_data_timeout) {
	...
	do {
		int rv = this->socket->read(buf, len, abstimeout.get_timeout());
		if (rv < 0) {
...
ssize_t Telnet::TelnetConsoleSvc::raw_write(const char *buf, size_t len, TickType_t timeout) {
	...
	TRACE.log(inattempt.data(), inattempt.size(), LogTree::LOG_TRACE, "A", 1, false);
	ssize_t rv = this->socket->send(buf, len, timeout);
	if (rv < 0) {
		if (errno != EAGAIN)
			this->close(); // Not a timeout.  Terminate.
		TRACE.log(inattempt.data(), inattempt.size(), LogTree::LOG_TRACE, "XXX", 3, false);
